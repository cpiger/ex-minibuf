...
function! <SID>BuildBufferList(delBufNum, updateBufList)
    ...
    while(l:i <= l:NBuffers)
        ...
        if (a:delBufNum == -1 || l:i != a:delBufNum)
            ...
            " if(getbufvar(l:i, '&buflisted') == 1 && '*vimshell*'!=bufname(l:i))
...
function! <SID>CycleBuffer(forward)
  ...
  while (getbufvar(l:curBuf, '&modifiable') == 0 && l:origBuf != l:curBuf) || bufname("%") == "" || bufname("%") == "-MiniBufExplorer-"
    if (a:forward == 1)
        bn!
    else
        bp!
